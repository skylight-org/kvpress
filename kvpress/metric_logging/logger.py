...
#
# Vendored from sparse_attention_hub/metric_logging/logger.py (Apache-2.0), with a
# test reset helper added for kvpress unit tests.
...
class MicroMetricLogger:
    ...
    def reset_for_testing(cls) -> None:
        """Clear the singleton so tests can reconfigure logging from a clean state.

        Registered metric identifiers are kept so re-importing packages is unnecessary.
        """
        cls._instance = None
